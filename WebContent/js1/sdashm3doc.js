...
var globalFileNames = [];
var globalUploadProcess = "";

sap.ui.model.json.JSONModel.extend("sdashm3doc", {
...
//								    	var ext = data.FileExt.toLowerCase();
		...
		deleteDocuments : function(jsonSDASHM3AllDocsForDeletion){
	    	...
	    	oModel = new sap.ui.model.odata.ODataModel(serviceDEP, true);
				...
				var jsonSDASHM3AllDocsForDeletionlength = jsonSDASHM3AllDocsForDeletion.length;
				if(jsonSDASHM3AllDocsForDeletionlength < 10){
					oSDASHM3TableDeleteDocument.setVisibleRowCount(jsonSDASHM3AllDocsForDeletionlength);
				}
				else{
					oSDASHM3TableDeleteDocument.setVisibleRowCount(10);
				}
		...
		openDeleteDocumentsPopup : function(button){
			...
			var oSDASHM3ButtonDeleteDocumentClose = new sap.ui.commons.Button({
							...
							press:function(oEvent){
								...
								var oSDASHM3ContentThumbNail = new sdashm3().setContentThumbnail(global3SerialNo);
								sap.ui.getCore().byId("idSDASHM3ContentFinal").insertItem(oSDASHM3ContentThumbNail, 5);
							}
					}).addStyleClass("marginTop10");//.addStyleClass("excelBtn");
			...
			oSDASHM3TableDeleteDocument.addColumn(new sap.ui.table.Column({
				 ...
				 for(var i=0; i<jsonSDASHM3Documents.length;i++){
					 jsonSDASHM3AllDocs.push({
						 ...
						 "dispname" : jsonSDASHM3Documents[i].dispname
					 });
				 }
		...
		openUploadDocumentsPopup : function(button, process){
			...
			var oSDASHM3ContentUploadDocument = osdashm3doc.loadSDASHM3UploadDocument(process);



			/* SDASHM3 - Popover - Upload Document */

			if(sap.ui.getCore().byId("idSDASHM3PopoverUploadDocument") != undefined)
				sap.ui.getCore().byId("idSDASHM3PopoverUploadDocument").destroy();

			oSDASHM3PopoverUploadDocument = new sap.m.Popover("idSDASHM3PopoverUploadDocument",{
		...
		getSDASHM3DataUploadNew : function(process){
			var oCurrent = this;
			...
			globalFileNames = oSDASHM3UploaderDocument.getValue().replace(/"/g, '').split(' ');
				...
				if(sap.ui.getCore().byId("idSDASHM3UploaderDocument").oFileUpload.files[0] == undefined){
					sap.ui.commons.MessageBox.show("Please select a file for upload");
					busyDialog.close();
					return;
				}

				var fileName = sap.ui.getCore().byId("idSDASHM3UploaderDocument").oFileUpload.files[0].name; //.split('.').pop();
				...
				if(process == "ADDEST"){
					...
					var arraySelLines = sap.ui.getCore().byId("idSDASHM2TableEquipmentLevel").getSelectedIndices();
			 		 ...
			 		 if(arraySelLines.indexOf(i) != -1){
			 			 ...
			 			 if(oDetData != undefined){
			 				 var realPath = oDetData.getPath().split('/')[2];
			 				 serial = oSDASHMJsonEquipmentLevel[realPath].serialno;
			 				 //equnrsim = oSDASHMJsonEquipmentLevel[realPath].equnr;
			 				 depot = oSDASHMJsonEquipmentLevel[realPath].depotcode;
			 				 estimate = oSDASHMJsonEquipmentLevel[realPath].estimateno;
			 			 }
			 		 }
			 	 }
				 var oSDASHM2 = new sdashm2();
				 oSDASHM2.submitSDASHM2();
				}else{
		...
		getSDASHM3DataUpload: function(){
			...
			loadSDASHM3UploadDocument : function(process) {
				var oCurrent = this;
				...
				var oSDASHM3UploaderDocument = new sap.ui.commons.FileUploader("idSDASHM3UploaderDocument",{
							//name : "uploadfileData",
							...
							change : function(oEvent) {
								...
								var files = oEvent.oSource.oFileUpload.files; // $('#osdashm3doc-fu');
								var valid = true;
								if (files != undefined) {	//INSIDE COMES FOR OTHER THAN IE BROWSER
									for ( var i = 0; i < files.length; i++) {
										...
										if($.inArray(ext, ['pdf', 'doc', 'docx', 'png', 'jpg', 'jpeg', 'txt', 'xls', 'xlsx', 'zip']) == -1) {
											valid = false;
								...
								if(valid){
									var oSDASHM3Doc = new sdashm3doc();
									var oSDASHM3DocTable = oSDASHM3Doc.createSDASHM3DocTable(oEvent);

									var oSDASHM3FormTable = sap.ui.getCore().byId("idSDASHM3FormTable");
									oSDASHM3FormTable.insertField(oSDASHM3DocTable, 0);
								}
							}
						});
				...
				var oSDASHM3ButtonUpload = new sap.ui.commons.Button("idSDASHM3ButtonUpload", {
							...
							layoutData: new sap.ui.layout.GridData({span: "L2 M2 S4",linebreak: false, margin: true}),
							press : function(){
									oCurrent.getSDASHM3DataUploadNew(process);
							}
						}).addStyleClass("marginTop10");//.addStyleClass("excelBtn");
		       ...
		       var lblSpaceLegend = new sap.ui.commons.Label( {text: " ",width : '5px'});
				...
				var oSDASHM3LabelSelectFiles =new sap.ui.commons.Label("idSDASHM3LabelSelectFiles", {text: "Select Files for Uploading: ",
					required:false}).addStyleClass("sapUiLbl floatLeft");
									...
									new sap.ui.layout.form.FormContainer(
											"SelFileUpldMultCert",
											{
												formElements : [
														...
														new sap.ui.layout.form.FormElement(
																{
																	fields : [oSDASHM3UploaderDocument],
																	layoutData: new sap.ui.layout.ResponsiveFlowLayoutData({linebreak: true, margin: false})
																}),
																new sap.ui.layout.form.FormElement(
																		{
														...
														new sap.ui.layout.form.FormElement(
																{
																	fields : [new sap.ui.commons.Label({text: "Support Format: pdf, doc, docx, png, jpg, jpeg, txt, xls, xlsx, zip"}).addStyleClass("font11 WarningMessage")],
																	layoutData: new sap.ui.layout.ResponsiveFlowLayoutData({linebreak: true, margin: false})
																})
																]
											}) ]
